...
const About = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (event) => {
      setLightPosition({ top: event.pageY - 250, left: event.pageX - 300 });
    };

    const handleScroll = () => {
      const lightElement = document.getElementById('light');
      const scrollTop = window.pageYOffset || document.documentElement.scrollTop;
      lightElement.style.top = `${lightPosition.top - scrollTop}px`;
    };
  ...
  return (
    <div>
      <div id='light' style={{ top: lightPosition.top, left: lightPosition.left }}></div>
      <div id="resumewrap">
